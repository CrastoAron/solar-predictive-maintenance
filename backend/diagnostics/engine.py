"""Public entry point for independent, explainable diagnostics."""
...
class DiagnosticsEngine:
    """Run deterministic root-cause analysis without side effects or ML labels."""
    ...
    def diagnose(
        ...
        # Retain the legacy call shape for scheduler/maintenance compatibility,
        # but deliberately never use these invalid classifier/rating inputs.
        del ml_prediction, panel_config
        ...
        context = DiagnosticContext(latest, history, _hardware_status(hardware_status), baseline_assessment)
        ...
        health = "Fault" if candidate.cause == "Sensor Failure" else _health_from_baseline(baseline_assessment)

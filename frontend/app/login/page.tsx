  ...
  return (
    ...
    >
      <input
        ...
        className="w-full rounded-xl border border-slate-700 bg-slate-900/70 px-4 py-3 text-sm text-white outline-none"
        required
      />
      <input
        type="password"
    ...
    <div className="min-h-screen bg-[#0f1117] flex items-center justify-center relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md mx-4">
        <div className="glass-card p-10 flex flex-col items-center text-center shadow-2xl">
          ...
          <div className="flex w-full gap-2 rounded-xl border border-slate-800 bg-slate-900/50 p-1">
            <button
              type="button"
              onClick={() => setMode("customer")}
              className={`flex-1 rounded-lg px-3 py-2 text-sm font-medium ${mode === "customer" ? "bg-white text-slate-900" : "text-slate-400"}`}
            >
              Customer
            </button>
            <button
              ...
              className={`flex-1 rounded-lg px-3 py-2 text-sm font-medium ${mode === "admin" ? "bg-orange-500 text-white" : "text-slate-400"}`}
            >
              Admin
            </button>
          </div>
          ...
          {mode === "customer" ? (
            <button
              ...
              disabled={loading}
              className="mt-6 w-full flex items-center justify-center gap-3 px-5 py-3.5 rounded-xl bg-white text-gray-700 font-semibold text-sm hover:bg-gray-50 active:bg-gray-100 transition-all duration-200 shadow-lg hover:shadow-xl disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {loading ? (
            ...
            <AdminLoginForm onSubmit={handleAdminSignIn} loading={loading} error={error} />
          )}
